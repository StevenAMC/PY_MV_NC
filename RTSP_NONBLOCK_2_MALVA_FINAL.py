...
rtsp_url1 = f"rtsp://admin:admin2025@{IP_camera1}:554/cam/realmonitor?channel=1&subtype=0?buffer_size=102400000"
rtsp_url2 = f"rtsp://admin:admin2025@{IP_camera2}:554/cam/realmonitor?channel=1&subtype=0?buffer_size=102400000"
...
class RTSP_movement:
    ...
    def detect_movement(self):
        ...
        frame = cv2.resize(
                            frame_org, None, fx=__SCALA__, fy=__SCALA__, interpolation=cv2.INTER_AREA
                        )
        ...
        num_objetos, etiquetas, estadisticas, centroides = cv2.connectedComponentsWithStats(fgmask)
        bounding_boxes = []
        
        for i in range(1, num_objetos):  # Comenzamos desde 1 para ignorar el fondo
            x, y, w, h, area = estadisticas[i]
            if area > __UMBRA_TAM__:
                #print("Área", i, ": ", area)
                bounding_boxes.append((x, y, w, h, area))
                cv2.rectangle(frame_show, (int(x/__SCALA__),int(y/__SCALA__)), (int((x+w)/__SCALA__), int((y+h)/__SCALA__)),(0,255,0), 2)
                #texto_estado = "Estado: Alerta Movimiento Detectado!"
                #color = (0, 0, 255)
                #break
        
        #if len(bounding_boxes) > 0:
        #    bounding_boxes = sorted(bounding_boxes, key=lambda bb: bb[4], reverse=True)
        #    x, y, w, h, area = estadisticas[1]
        #    if area >__UMBRA_TAM__:
        #        pass
                """try:
        ...
        cv2.putText(frame_show, f"FPS: {fps:.2f}" , (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255),2)
...
serial_app = SerialSender(cola=cola_datos, port='/dev/ttyAMA0')
...
while True:
    ...
    if not stream.get_running() or not stream2.get_running() or exit_event.is_set():
        print("ERROR")
        for c in conexiones:
            try:
                c.stop()
    ...
    if frame is not None and frame2 is not None:
        ...
        """frame = stream.process_video(frame, ocr)
        frame2 = stream2.process_video(frame2, ocr)"""
