...
"""rtsp_url1 = (
    f"rtspsrc location=rtsp://admin:admin2025@{IP_camera3}:554/cam/realmonitor?channel=1&subtype=0?buffer_size=102400000 latency=0 ! "
    "rtph265depay ! h265parse ! nvv4l2decoder ! nvvidconv ! videorate ! "
    "video/x-raw, format=BGRx, framerate=3/1 ! appsink"
)"""
rtsp_url2 = (
...
class ClienteTCP_Sender:
    def __init__(self,cola, host='192.168.18.180', port=65432):
        self.host = host
        self.port = port
        self.mensajes = cola
        self.running = False
...
cliente.iniciar()
baliza = Baliza(cola_datos,intervalo=295)  # 5 minutos
baliza.iniciar()
